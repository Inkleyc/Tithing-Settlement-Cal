...
async function responseFor(token:string){const repository=getRepository();const appointment=await repository.getAppointmentByToken(token);if(!appointment)return null;return{appointment,currentSlot:await repository.getAppointmentSlot(appointment),availableSlots:(await repository.getAllPublicSlots()).filter((s)=>!s.isReserved&&!s.isBuffer&&!s.isBlocked)};}

...
describe("appointment email templates",()=>{
  ...
  it("confirms the new details after a reschedule",()=>{const message=rescheduleConfirmationEmail(details);expect(message.subject).toContain("appointment updated");expect(message.text).toContain("rescheduled and confirmed");expect(message.text).toContain("New date: Sunday, October 18, 2026");expect(message.text).toContain("New time: 4:40 PM–5:00 PM");expect(message.html).toContain("Reschedule again");});
});

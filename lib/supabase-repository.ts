...
export const supabaseRepository: ScheduleRepository = {
  ...
  async toggleSlotBuffer(id) { const client = createSupabaseAdminClient(); const [current, bookings] = await Promise.all([client.from("time_slots").select("is_buffer").eq("id", id).maybeSingle(), client.from("appointments").select("id").eq("status", "confirmed").or(`time_slot_id.eq.${id},paired_slot_id.eq.${id}`).limit(1)]); fail(current.error); fail(bookings.error); if (!current.data) throw new Error("Selected time slot no longer exists."); if (bookings.data?.length) throw new Error("A reserved slot cannot be changed to or from a buffer."); const result = await client.from("time_slots").update({ is_buffer: !current.data.is_buffer, is_blocked: false }).eq("id", id).select().single(); fail(result.error); return result.data; },
